...
rm ./docker/mysql/2-schema.sql 2>/dev/null
rm ./docker/mysql/3-data.sql 2>/dev/null

# Adding this line in every script because MySQL throws an error when the Database is not selected in every Script
sed '1 s/^/USE \`microservice\`;/' ./src/main/resources/db/schema.sql > ./docker/mysql/2-schema.sql
sed '1 s/^/USE \`microservice\`;/' ./src/main/resources/db/data.sql > ./docker/mysql/3-data.sql

echo "Building Docker Image"
docker build -t "$IMAGE_NAME:$IMAGE_TAG" ./docker/mysql/

echo "creating the Docker Network"
docker network create $NETWORK

echo "Running the Container"
docker run -d --name=$CONTAINER_NAME -p 3306:3306 -e MYSQL_ROOT_PASSWORD=$MYSQL_ROOT_PASSWORD --network $NETWORK $IMAGE_NAME:$IMAGE_TAG
